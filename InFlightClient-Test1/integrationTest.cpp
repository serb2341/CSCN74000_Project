...
TEST(IntegrationTests, Packet_RoundTrip_Serialize_Validate_Deserialize)
{
    // Arrange
    InFlightClient client;
    Packet original;
...
TEST(IntegrationTests, Packet_CRC_Integrity_IsPreserved_Across_Serialize)
{
    // Arrange
    Packet pkt;
    pkt.SetFlightID(7);
    ...
    std::memcpy(&extractedCRC,
        buffer + sizeof(PacketHeader) + pkt.GetBodyLength(),
        sizeof(uint32_t));
...
TEST(IntegrationTests, Packet_SentMessage_IsLoggedCorrectly)
{
    // Arrange
...
TEST(IntegrationTests, Client_LoadConfig_And_ComputeSignature_WorkTogether)
{
    // Arrange
    InFlightClient client;
...
TEST(IntegrationTests, FullPipeline_Packet_IsValidated_And_Logged)
{
    // Arrange
    InFlightClient client;
