...
//  Shared constants
// ============================================================
static constexpr unsigned short TEST_PORT = 55555;
static constexpr unsigned short TEST_PORT_A = 55600;   // Used for two-listener tests
static constexpr unsigned short TEST_PORT_B = 55601;
...
//  Both methods are private; access is granted via friend.
// ============================================================
TEST_CLASS(Test_Handshake_Integration) {

    // Helper: creates a bound + listening TCP socket on 'port'.
    // The caller owns the returned SOCKET and must closesocket() it.
    static SOCKET MakeListenSocket(unsigned short port) {
        ...
        if (bind(s, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR) {
            (void)closesocket(s); return INVALID_SOCKET;
        }
        (void)listen(s, 2);
        return s;
    }

    // Helper: connects a raw TCP socket to localhost:port.
...
public:
    ...
    //
    //  Server side  -> Networking::Server::PerformHandshake()
    //  Client side  -> Client::InFlightClient::PerformHandshake()
    // ----------------------------------------------------------
    TEST_METHOD(Test1_CorrectSecret_HandshakeSucceeds) {
        SOCKET listenSock = MakeListenSocket(TEST_PORT);
        Assert::IsTrue(listenSock != INVALID_SOCKET,
            L"Test 1: Listening socket creation must succeed");

        Networking::Server server;
        server.sharedSecret = TEST_SECRET;     // set via friend access

        std::atomic<bool> serverResult{ false };

        // Server thread: accept one connection, run server-side handshake.
        ...
        // Client side: InFlightClient connects and runs its handshake.
        Client::InFlightClient ifClient;
        ifClient.sharedSecret = TEST_SECRET;   // set via friend access

        SOCKET clientSock = MakeClientSocket(TEST_PORT);
    ...
    //         shared secret — Server::PerformHandshake rejects
    //         the signature in Step 4.
    // ----------------------------------------------------------
    TEST_METHOD(Test2_WrongClientSecret_HandshakeFails) {
        ...
        Client::InFlightClient ifClient;
        ifClient.sharedSecret = "COMPLETELY_WRONG_SECRET";   // deliberate mismatch
        ...
        // At least one side must detect the signature mismatch.
    ...
    TEST_METHOD(Test3_TamperedChallengeCRC_ServerRejectsHandshake) {
        ...
        Handshake::ChallengePacket ch{};
        ch.Type = static_cast<uint32_t>(Handshake::VerificationPacketType::CHALLENGE);
    ...
    //         because the computed signatures will not match.
    // ----------------------------------------------------------
    ...
    //         Uses MutualVerification::Handshake::Execute()
    //         (the static public method called by GC internally)
    //         against Server::PerformHandshake().
    // ----------------------------------------------------------
    TEST_METHOD(Test5_GroundControlClient_HandshakeSucceeds) {
        ...
        // Connect a raw socket and run the GC-side handshake logic directly.
        // MutualVerification::Handshake::Execute is a static public method so
        // no friend access is needed here.
        SOCKET gcSock = MakeClientSocket(TEST_PORT);
        Assert::IsTrue(gcSock != INVALID_SOCKET,
            L"Test 5: GC socket must connect");

        bool gcResult = MutualVerification::Handshake::Execute(gcSock, TEST_SECRET);
...
//  (via friend) to verify integrity.
// ============================================================
TEST_CLASS(Test_PacketRelay_Integration) {

    static SOCKET MakeListenSocket(unsigned short port = TEST_PORT) {
        SOCKET s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
        int opt = 1;
        (void)setsockopt(s, SOL_SOCKET, SO_REUSEADDR,
            reinterpret_cast<const char*>(&opt), sizeof(opt));
        ...
        addr.sin_addr.s_addr = INADDR_ANY;
        addr.sin_port = htons(port);
        (void)bind(s, reinterpret_cast<sockaddr*>(&addr), sizeof(addr));
        (void)listen(s, 2);
        return s;
    }

    static SOCKET MakeClientSocket(unsigned short port = TEST_PORT) {
        SOCKET s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
        sockaddr_in addr{};
        addr.sin_family = AF_INET;
        addr.sin_port = htons(port);
    ...
    // Builds a serialised packet using the real Communication::Packet class.
    ...
    // Receives a full packet (header + body + CRC) from 'sock' into a vector.
    static std::vector<char> RecvFullPacket(SOCKET sock) {
        ...
        if (recv(sock, headerBuf, sizeof(headerBuf), MSG_WAITALL)
            != sizeof(Communication::PacketHeader))
            return {};
...
public:
    ...
    TEST_METHOD(Test6_InFlightToGC_PacketArrivesIntact) {
        ...
        // GC / server-side receiver thread.
        std::thread receiverThread([&]() {
            ...
            // Use the real Server::ValidatePacket to check integrity.
        ...
        // InFlight sender builds packet via real Packet class.
        ...
        // Deserialise using the real Packet class and check field values.
        Communication::Packet rxPkt(receivedBuf.data());
    ...
    //         InFlightClient::ValidatePacket() is used to verify
    //         structural and CRC integrity on the client side.
    // ----------------------------------------------------------
    TEST_METHOD(Test7_GCReply_ReceivedAndValidatedByInFlight) {
        ...
        // InFlight receiver thread: waits for an incoming packet.
        std::thread inFlightThread([&]() {
            ...
            // Use the real InFlightClient::ValidatePacket.
            if (!receivedBuf.empty()) {
        ...
        // GC sender builds packet via real Packet class.
    ...
    TEST_METHOD(Test8_CorruptedPacketBody_RejectedByServerValidation) {
        ...
        // Build a valid packet then corrupt one byte inside the body.
        auto pktBytes = BuildPacket(404, 0, "Uncorrupted body text here");
        pktBytes[sizeof(Communication::PacketHeader) + 2] ^= 0xFF;   // flip bits
    ...
    TEST_METHOD(Test9_CorruptedPacket_RejectedByInFlightValidation) {
        ...
        pktBytes[sizeof(Communication::PacketHeader) + 3] ^= 0xAB;   // corrupt
...
public:
    ...
    //          Both GC-received message and InFlight-received
    //          reply must match exactly.
    // ----------------------------------------------------------
    TEST_METHOD(Test10_RequestReply_ThroughValidatingRelay) {
        // Two listening sockets simulate two separate accept paths.
        SOCKET listenA = MakeListenSocket(TEST_PORT_A);  // InFlight connects here
        SOCKET listenB = MakeListenSocket(TEST_PORT_B);  // GC connects here
        ...
        // Relay thread: accept from both sides, validate with Server::ValidatePacket
        // before forwarding in each direction.
        std::thread relayThread([&]() {
            SOCKET sockA = accept(listenA, nullptr, nullptr);  // InFlight side
            SOCKET sockB = accept(listenB, nullptr, nullptr);  // GC side
            if (sockA == INVALID_SOCKET || sockB == INVALID_SOCKET) return;

            // Forward A -> B (InFlight -> GC), validate first.
            {
            ...
            // Forward B -> A (GC -> InFlight), validate first.
            {
        ...
        // GC thread: connects, receives the InFlight message, then replies.
        std::thread gcThread([&]() {
            SOCKET gcSock = MakeClientSocket(TEST_PORT_B);
            if (gcSock == INVALID_SOCKET) return;

            auto buf = RecvFullPacket(gcSock);
            if (!buf.empty()) {
                Communication::Packet rxPkt(buf.data());
                gcReceived = std::string(rxPkt.GetData(), rxPkt.GetBodyLength());

                // GC replies using the real Packet class.
        ...
        // InFlight: send the initial message, then block waiting for GC reply.
        SOCKET ifSock = MakeClientSocket(TEST_PORT_A);
        ...
        if (!replyBuf.empty()) {
            Communication::Packet replyPkt(replyBuf.data());
    ...
    //          order and with the correct content, each passing
    //          Server::ValidatePacket before being accepted.
    // ----------------------------------------------------------
    TEST_METHOD(Test11_MultipleMessages_OrderAndContentPreserved) {
        ...
        std::thread receiverThread([&]() {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                // Validate with the real server method before reading content.
                if (!server.ValidatePacket(buf.data(),
                    static_cast<unsigned int>(buf.size()))) break;

                Communication::Packet pkt(buf.data());
                received.push_back(
        ...
        SOCKET senderSock = MakeClientSocket(TEST_PORT);
        Assert::IsTrue(senderSock != INVALID_SOCKET,
...
//
//  Tests MessageType = 1 (telemetry) end-to-end using the real
//  Packet class and Server::ValidatePacket.
// ============================================================
...
public:
    ...
    //          passes Server::ValidatePacket, and the MessageType
    //          field is correctly identified on the receiving end.
    // ----------------------------------------------------------
    TEST_METHOD(Test12_TelemetryPacket_ValidatedAndIdentified) {
        ...
        // InFlight client sends a telemetry packet using the real Packet class.
    ...
    TEST_METHOD(Test13_LargeTelemetryPayload_TransmittedAndValidated) {
        ...
        // Build an ~8 KB telemetry string.
        ...
        std::thread gcThread([&]() {
            ...
            // Stream-receive until the full packet arrives.
            char headerBuf[sizeof(Communication::PacketHeader)];
...
//
//  Tests that recv() returns 0 on graceful disconnect and that
//  the server port is immediately reusable for a second client.
// ============================================================
...
public:
    ...
    TEST_METHOD(Test14_GracefulClientDisconnect_DetectedByServer) {
        ...
        std::thread serverThread([&]() {
            ...
            disconnectDetected = (bytes == 0);   // 0 = graceful close
    ...
    //          successfully — two sequential validated sessions.
    // ----------------------------------------------------------
    TEST_METHOD(Test15_ClientReconnect_SecondSessionSucceeds) {
        ...
        // Server thread: accept and serve two sequential clients.
        std::thread serverThread([&]() {
            for (int s = 0; s < 2; ++s) {
                SOCKET conn = accept(listenSock, nullptr, nullptr);
        ...
        // First client — connect, send one validated packet, then disconnect.
        {
        ...
        // Second client — reconnects on the same port.
        {
        ...
        Assert::AreEqual(static_cast<size_t>(2), serverReceived.size(),
            L"Test 15: Two messages must be received and validated across both sessions");
...
//
//  Tests that FlightID set by Packet::SetFlightID() is
//  preserved end-to-end through SerializeData / Packet(char*)
//  and confirmed by Server::ValidatePacket.
// ============================================================
...
public:
    ...
    TEST_METHOD(Test16_FlightID_PreservedEndToEnd) {
        ...
        // Build packet using the real Packet class.
        Communication::Packet txPkt;
    ...
    //          correctly received, pass Server::ValidatePacket,
    //          and are distinguished in order.
    // ----------------------------------------------------------
...
//
//  Tests a realistic full session: Server::PerformHandshake
//  first on the same socket, then real Packet exchange +
//  Server::ValidatePacket on the data that follows.
// ============================================================
...
public:
    ...
    //          Realistic full session: Server::PerformHandshake
    //          followed by Server::ValidatePacket on data.
    // ----------------------------------------------------------
    TEST_METHOD(Test18_HandshakeThenDataPacket_FullSessionSucceeds) {
        ...
        std::thread serverThread([&]() {
            ...
            // Handshake phase using the real Server::PerformHandshake.
            if (!server.PerformHandshake(conn, "InFlightClient")) {
                (void)closesocket(conn); return;
            }

            // Data phase: receive and validate using Server::ValidatePacket.
            auto buf = RecvFullPacket(conn);
            if (!buf.empty() &&
                server.ValidatePacket(buf.data(),
                    static_cast<unsigned int>(buf.size()))) {
        ...
        // Client side: InFlightClient handshake then send a data packet.
        Client::InFlightClient ifClient;
        ...
        Assert::IsTrue(hsOk,
            L"Test 18: InFlightClient::PerformHandshake must succeed before data is sent");

        // Send data using the real Packet class.
        Communication::Packet txPkt;
    ...
    // ----------------------------------------------------------
    // Test 19: When Server::PerformHandshake fails due to a
    //          wrong client secret, the server drops the socket
    //          immediately and no data exchange occurs.
    // ----------------------------------------------------------
    TEST_METHOD(Test19_FailedHandshake_NoDataExchangeOccurs) {
        SOCKET listenSock = MakeListenSocket();
        Assert::IsTrue(listenSock != INVALID_SOCKET,
            L"Test 19: Listening socket must be created");
        ...
        std::thread serverThread([&]() {
            ...
            bool hsOk = server.PerformHandshake(conn, "InFlightClient");
            handshakeRejected = !hsOk;

            if (!hsOk) {
                // Drop immediately — no data phase entered.
                (void)closesocket(conn);
                return;
            }
        ...
        Client::InFlightClient ifClient;
        ifClient.sharedSecret = "COMPLETELY_WRONG";

        SOCKET clientSock = MakeClientSocket();
        Assert::IsTrue(clientSock != INVALID_SOCKET,
            L"Test 19: Client must connect");

        bool hsOk = ifClient.PerformHandshake(clientSock, "InFlightClient");

        // Attempt to send a data packet regardless of the handshake outcome.
        Communication::Packet txPkt;
        txPkt.SetFlightID(1);
        txPkt.SetMessageType(0);
        txPkt.SetData("Should not arrive", 17);
        unsigned int sz = 0;
        char* raw = txPkt.SerializeData(sz);
        (void)send(clientSock, raw, static_cast<int>(sz), 0);
        ...
        Assert::IsTrue(handshakeRejected,
            L"Test 19: Server::PerformHandshake must return false for a wrong secret");
        Assert::IsFalse(dataReceived,
            L"Test 19: No data must be received by the server after a rejected handshake");
    }
};