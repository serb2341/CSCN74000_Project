 ...
 */
GroundControlClient::GroundControlClient()
 ...
 */
bool GroundControlClient::Initialize() {
    // Attempt to load the security credentials from the configuration file
    if (!LoadConfig("server_config.txt")) return false;
 ...
 */
bool GroundControlClient::LoadConfig(const std::string& configPath) {
    // Extract the shared secret to be used during the validation handshake
    sharedSecret = Handshake::LoadSecret(configPath);
...
bool GroundControlClient::InitializeWinsock() {
    WSADATA wsaData;
 ...
 */
bool GroundControlClient::CreateSocket() {
 ...
 */
void GroundControlClient::ValidateConnection() {
    // Run the 4-packet exchange to verify both the server and the client
    if (!Handshake::Execute(clientSocket, sharedSecret)) {
 ...
 */
void GroundControlClient::sendMessage(int messageType, const std::string& message) {
    Packet txPkt;
    ...
    send(clientSocket, txData, size, 0);
 ...
 */
void GroundControlClient::receiveMessage() {
    // Retrieve the header to identify the packet type and length
    char headerBuffer[sizeof(PacketHeader)];
    int bytes = recv(clientSocket, headerBuffer, sizeof(headerBuffer), MSG_WAITALL);
    ...
    PacketHeader head;
    std::memcpy(&head, headerBuffer, sizeof(PacketHeader));
    activeFlightID = head.FlightID;

    // Create a buffer on the heap sized for this specific packet (dynamic allocation)
    unsigned int fullSize = sizeof(PacketHeader) + head.Length + sizeof(uint32_t);
    ...
    if (ValidatePacket(buffer, fullSize)) {
        Packet rxPkt(buffer);
